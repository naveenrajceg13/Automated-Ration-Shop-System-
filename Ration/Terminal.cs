  ...
  public partial class frmTerminal : Form
  {
    ...
    public void SendData(string msg)
    {
        ...
        else
        {
            ...
            {
                MessageBox.Show("LED display is not connected or some problem with it ");
            }
        }
        ...
        else
        {
            ...
            {
                // Inform the user if the hex string was not properly formatted
                Log(LogMsgType.Error, "Not properly formatted hex string: " + msg + "\n");
            }
        }
        txtSendData.SelectAll();
    }
    ...
    { e.Handled = KeyHandled; }
      ...
      {
          if (rtfTerminal.Text.Length > 0)
          {
              Directory.CreateDirectory(Application.StartupPath + "\\RFID Number");
              StreamWriter sw1 = new StreamWriter(Application.StartupPath + "\\RFID Number\\FromRFID.txt");
              sw1.Write(rtfTerminal.Text);
              sw1.Close();
              comport.Dispose();
              this.Close();
              timer1.Stop();
