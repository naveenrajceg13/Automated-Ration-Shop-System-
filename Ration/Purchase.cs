...
namespace Ration
{
    public partial class Purchase : Form
    {
        ...
        private void check_stock()
        {
            ...
            while (dr.Read())
            {
                String sname=dr["PName"].ToString();
                String squnty=dr["Quantity"].ToString();
                int intsqunty = Convert.ToInt32(squnty);
                if (intsqunty < 100)
                {
                    MessageBox.Show(sname + " quantity is low in stock ");
                }
            }
            con.Close();
        }
        ...
        private void sendMessage(string mobileNo,string msg)
        {
            try
            {
                ...
                MessageBox.Show("Message send to ,   " + mobileNo);
            }
            catch (Exception ex)
            {

                MessageBox.Show("Message sending failed,   "+mobileNo);
            }
